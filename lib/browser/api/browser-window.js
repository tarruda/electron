...
BrowserWindow.prototype._init = function () {
  let pendingWebContents = new Map()
  ...
  this.webContents.on('-new-web-contents', (event, webContents, url,
                                            frameName) => {
    pendingWebContents.set(webContents, {url, frameName});
  })
  // Create a new browser window for the native implementation of
  // "window.open"(sandboxed mode only)
  this.webContents.on('-new-native-window', (event, webContents, disposition,
                                             userGesture, left, top, width,
                                             height) => {
    let {url, frameName} = pendingWebContents.get(webContents);
    pendingWebContents.delete(webContents);
    const options = {
      show: true,
      x: left,
      y: top,
    ...
    ipcMain.emit('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_OPEN', event, url,
