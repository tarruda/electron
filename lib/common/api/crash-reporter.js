...
class CrashReporter {
  getBindingStartArgs (options) {
    ...
    return [this.getProductName(), companyName, submitURL, this.getCrashesDirectory(),
      uploadToServer, ignoreSystemCrashHandler, extra]
  }

  ensureInternalCrashService (submitURL, productName, crashesDirectory) {
  ...
  start (options) {
    let [productName, companyName, submitURL, crashesDirectory, uploadToServer,
      ignoreSystemCrashHandler, extra] = this.getBindingStartArgs(options)

    this.ensureInternalCrashService(submitURL, productName, crashesDirectory)
